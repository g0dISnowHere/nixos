let
  ...
  monitoredHosts = builtins.filter (host: host.monitoring_enabled) (builtins.attrValues hosts);
  ...
  group = predicate: sort (map (host: host.hostname) (builtins.filter predicate monitoredHosts));
  ...
  validateHost =
    ...
    true;
in
assert builtins.all validateHost monitoredHosts;
{
  inherit hosts;
